...
const Settings = () => {
  ...
  const InputField = ({ label, ...props }) => (
    <div>
      ...
      <input
        className="w-full px-3 sm:px-4 py-2 sm:py-2.5 border border-gray-700 bg-black text-gray-100 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none transition-all"
        {...props}
      />
    </div>
  );
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-4 sm:space-y-6 bg-black text-gray-100 p-3 sm:p-4 md:p-6 rounded-2xl">
      <motion.div
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
      >
        {/* Header */}
        <div className="bg-gradient-to-r from-indigo-600 to-purple-600 rounded-2xl p-4 sm:p-5 md:p-6 text-white mb-4 sm:mb-6">
        ...
        {/* Profile preview card */}
        <div className="bg-gray-950 rounded-2xl border border-gray-800 shadow-sm p-4 sm:p-5 mb-4 sm:mb-6 flex flex-col sm:flex-row items-center sm:items-start gap-4 sm:gap-6">
          <div className="relative flex-shrink-0">
            <img
              src={currentAvatar}
              alt={user?.name}
              className="w-16 h-16 sm:w-20 sm:h-20 rounded-2xl object-cover ring-4 ring-indigo-900/40 border border-gray-700"
              onError={(e) => {
                e.target.src = getAvatarUrl(user?.name, null);
              }}
            />
            <label className="absolute -bottom-1 -right-1 w-5 h-5 sm:w-6 sm:h-6 bg-indigo-600 rounded-full flex items-center justify-center cursor-pointer hover:bg-indigo-700 transition-colors">
              <svg
          ...
          {avatarFile && (
            <button
              onClick={handleAvatarUpload}
              disabled={loading}
              className="w-full sm:w-auto px-4 py-2 bg-indigo-600 text-white rounded-xl text-sm font-medium hover:bg-indigo-700 transition-colors disabled:opacity-50"
            >
              {loading ? "Uploading..." : "Save Photo"}
            </button>
          )}
        ...
        <div className="bg-gray-950 rounded-2xl border border-gray-800 shadow-sm overflow-hidden">
          <div className="flex overflow-x-auto border-b border-gray-800 scrollbar-hide">
            {tabs.map((tab) => (
              <button
                key={tab.id}
                onClick={() => setActiveTab(tab.id)}
                className={`flex-1 flex items-center justify-center gap-1.5 sm:gap-2 px-2 sm:px-4 py-3 sm:py-3.5 text-xs sm:text-sm font-medium transition-all border-b-2 whitespace-nowrap ${
                  activeTab === tab.id
                    ? "border-indigo-500 text-indigo-300 bg-indigo-900/30"
                    : "border-transparent text-gray-400 hover:text-gray-200 hover:bg-gray-900"
                }`}
          ...
          <div className="p-4 sm:p-5 md:p-6">
              ...
              >
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={3}
                    placeholder="Tell clients about yourself and your expertise..."
                    className="w-full px-3 sm:px-4 py-2 sm:py-2.5 border border-gray-700 bg-black text-gray-100 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none resize-none transition-all"
                  />
                  <p className="text-xs text-gray-500 mt-1">
                    {profileData.bio.length}/300 characters
                  </p>
                      ...
                      {profileData.skills.split(",").map(
                        (s, i) =>
                          s.trim() && (
                            <span
                              key={i}
                              className="px-2 sm:px-2.5 py-0.5 sm:py-1 bg-indigo-900/40 text-indigo-300 text-[10px] sm:text-xs rounded-full border border-indigo-700"
                            >
                              {s.trim()}
                            </span>
                          ),
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full py-2 sm:py-2.5 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-xl font-medium text-sm hover:shadow-lg transition-all duration-200 disabled:opacity-50"
                >
                  {loading ? "Saving..." : "Save Profile"}
                </button>
              </motion.form>
            ...
            {activeTab === "bank" && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                <div className="flex flex-col sm:flex-row items-start gap-2 sm:gap-3 p-3 sm:p-4 bg-blue-950/40 rounded-xl border border-blue-900 mb-4 sm:mb-5">
                  <span className="text-blue-500 text-base sm:text-lg flex-shrink-0">ℹ️</span>
                  <div>
                    <p className="text-sm font-semibold text-blue-300">
                      Secure Payment Info
                    </p>
                ...
                <form onSubmit={handleBankUpdate} className="space-y-3 sm:space-y-4">
                  <InputField
                    label="Account Holder Name"
                    type="text"
                    value={bankData.accountHolderName}
            ...
            {activeTab === "security" && (
              <motion.div
                ...
                className="space-y-4 sm:space-y-5"
              >
                <div className="p-3 sm:p-4 bg-yellow-950/40 rounded-xl border border-yellow-900">
                  <p className="text-sm font-semibold text-yellow-300 mb-2">
                    🔒 Security Tips
                  </p>
                  <ul className="text-xs text-yellow-400 space-y-1 list-disc list-inside">
                ...
                <form onSubmit={handlePasswordChange} className="space-y-3 sm:space-y-4">
                  <h3 className="font-semibold text-white text-sm sm:text-base">Change Password</h3>
                  <InputField
                    label="Current Password"
                    type="password"
                    value={passwordData.currentPassword}
