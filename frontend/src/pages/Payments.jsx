...
const Payments = () => {
  ...
  return (
    <div className="space-y-4 sm:space-y-6 max-w-5xl mx-auto px-3 sm:px-4 md:px-0">
      {/* Header */}
      <div className="bg-gradient-to-r from-indigo-600 to-purple-600 rounded-2xl p-4 sm:p-5 md:p-6 text-white">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          className="bg-gray-900 rounded-2xl p-4 sm:p-5 border border-gray-800 shadow-sm"
        >
          <div className="flex items-center gap-2 sm:gap-3 mb-2">
            <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-amber-50 flex items-center justify-center text-lg sm:text-xl">
              ⏳
            </div>
            <p className="text-xs sm:text-sm text-gray-400 font-medium">Pending Payment</p>
          </div>
        ...
        >
          <div className="flex items-center gap-2 sm:gap-3 mb-2">
            <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-green-50 flex items-center justify-center text-lg sm:text-xl">
              ✅
            </div>
            <p className="text-xs sm:text-sm text-gray-400 font-medium">Total Paid</p>
          </div>
        ...
        <motion.div
          ...
          animate={{ opacity: 1, y: 0 }}
          transition={{ delay: 0.2 }}
          className="bg-gray-900 rounded-2xl p-4 sm:p-5 border border-gray-800 shadow-sm sm:col-span-2 lg:col-span-1"
        >
          <div className="flex items-center gap-2 sm:gap-3 mb-2">
            <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-indigo-50 flex items-center justify-center text-lg sm:text-xl">
              💵
            </div>
            <p className="text-xs sm:text-sm text-gray-400 font-medium">Total Volume</p>
          </div>
        ...
        <div className="flex border-b border-gray-800">
          {["pending", "paid"].map((tab) => (
            <button
              key={tab}
              onClick={() => setActiveTab(tab)}
              className={`flex-1 py-3 sm:py-3.5 text-xs sm:text-sm font-medium transition-all border-b-2 capitalize ${
                activeTab === tab
                  ? "border-indigo-500 text-indigo-300 bg-indigo-500/10"
                  : "border-transparent text-gray-400 hover:text-gray-200"
              }`}
          ...
          {displayList.length === 0 ? (
            ...
            displayList.map((contribution, i) => (
              <motion.div
                ...
                animate={{ opacity: 1, x: 0 }}
                transition={{ delay: i * 0.05 }}
                className="flex flex-col sm:flex-row sm:items-start lg:items-center gap-3 sm:gap-4 p-3 sm:p-4 rounded-xl border border-gray-800 hover:border-indigo-500/40 hover:bg-gray-800/40 transition-all"
              >
                {/* Freelancer Avatar */}
                <img
                  src={getAvatarUrl(
                  ...
                  alt={contribution.freelancerId?.name}
                  className="w-10 h-10 sm:w-11 sm:h-11 rounded-xl object-cover flex-shrink-0 border border-gray-700"
                  onError={(e) => {
                    e.target.src = getAvatarUrl(
                      contribution.freelancerId?.name,
                      null,
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {user?.role === "client" &&
                    ...
                    contribution.freelancerId?.bankDetails
                      ?.accountHolderName && (
                      <div className="mt-2 p-2 sm:p-2.5 bg-blue-500/10 rounded-lg border border-blue-500/30 text-[10px] sm:text-xs text-blue-300 space-y-0.5">
                        <p className="font-semibold">💳 Payment Details</p>
                ...
                <div className="flex-shrink-0 self-start sm:self-center">
                  ...
                  user?.role === "client" ? (
                    <button
                      onClick={() => handlePay(contribution._id)}
                      disabled={payingId === contribution._id}
                      className="w-full sm:w-auto px-3 sm:px-4 py-1.5 sm:py-2 bg-gradient-to-r from-green-600 to-emerald-600 text-white text-[10px] sm:text-xs font-semibold rounded-xl hover:shadow-lg transition-all duration-200 disabled:opacity-50 whitespace-nowrap"
                    >
                      {payingId === contribution._id ? (
                    ...
                    <span
                      className={`px-2 sm:px-3 py-1 sm:py-1.5 rounded-full text-[10px] sm:text-xs font-semibold whitespace-nowrap ${
                        contribution.status === "paid"
                          ? "bg-green-500/20 text-green-300"
                          : "bg-amber-500/20 text-amber-300"
                      }`}
