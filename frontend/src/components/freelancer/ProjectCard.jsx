...
const ProjectCard = ({ project, index }) => {
  ...
  return (
    ...
    >
      <Link to={`/projects/${project._id}`} className="block h-full">
        <div className="relative bg-gradient-to-br from-gray-900 to-black rounded-xl border border-gray-800 hover:border-purple-500/50 transition-all duration-300 overflow-hidden h-full flex flex-col">
          {/* Glow Effect on Hover */}
          <div className="absolute inset-0 bg-gradient-to-r from-purple-600/0 via-purple-600/0 to-purple-600/0 group-hover:from-purple-600/5 group-hover:via-purple-600/5 group-hover:to-purple-600/5 transition-all duration-500"></div>
          ...
          <div className="absolute top-2 sm:top-3 right-2 sm:right-3 z-10">
            <span
              className={`inline-flex items-center gap-1 sm:gap-1.5 px-1.5 sm:px-2.5 py-0.5 sm:py-1 rounded-lg text-[10px] sm:text-xs font-medium border ${status.classes}`}
            >
          ...
          {/* Image Section */}
          {project.images && project.images.length > 0 && firstImageUrl && (
            <div className="relative h-24 sm:h-28 md:h-32 overflow-hidden flex-shrink-0">
              <img
                src={firstImageUrl}
                alt={project.title}
                className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500"
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-gray-900 via-transparent to-transparent"></div>
              {project.images.length > 1 && (
                <span className="absolute bottom-1 sm:bottom-2 right-1 sm:right-2 bg-black/80 text-white text-[10px] sm:text-xs px-1.5 sm:px-2 py-0.5 rounded-full backdrop-blur-sm">
                  +{project.images.length - 1}
                </span>
              )}
            </div>
          ...
          <div className="p-3 sm:p-4 flex-1 flex flex-col">
            ...
            <div className="mb-1.5 sm:mb-2">
              <span
                className={`inline-flex items-center gap-1 sm:gap-1.5 px-1.5 sm:px-2 py-0.5 rounded-lg text-[10px] sm:text-xs font-medium ${catColor.bg} ${catColor.text} border ${catColor.border}`}
              >
            ...
            {/* Title */}
            <h3 className="text-sm sm:text-base font-bold text-white mb-1 line-clamp-1 group-hover:text-purple-400 transition-colors">
              {project.title}
            </h3>

            {/* Description */}
            <p className="text-xs text-gray-500 line-clamp-2 mb-2 sm:mb-3 flex-1">
              {project.description}
            </p>
            ...
            {project.skills && project.skills.length > 0 && (
              <div className="flex flex-wrap gap-1 mb-2 sm:mb-3">
                {project.skills.slice(0, 2).map((skill, i) => (
                  <span
                    key={i}
                    className="px-1 sm:px-1.5 py-0.5 bg-gray-800 text-gray-400 text-[10px] sm:text-xs rounded-md truncate max-w-[60px] sm:max-w-[80px]"
                  >
                    {skill}
                  </span>
                ))}
                {project.skills.length > 2 && (
                  <span className="px-1 sm:px-1.5 py-0.5 bg-gray-800 text-gray-500 text-[10px] sm:text-xs rounded-md">
                    +{project.skills.length - 2}
                  </span>
                )}
              </div>
            ...
            {/* Footer */}
            <div className="flex items-center justify-between pt-2 sm:pt-3 border-t border-gray-800 mt-auto">
              <div className="flex items-center gap-1.5 sm:gap-2 min-w-0">
                <div className="relative flex-shrink-0">
                  <img
                    src={ownerAvatar}
                    alt={ownerName}
                    className="w-5 h-5 sm:w-6 sm:h-6 rounded-full object-cover ring-2 ring-gray-800"
                    onError={(e) => {
                      e.target.src = getAvatarUrl(ownerName, null);
                    }}
                  />
                  <div className="absolute -bottom-0.5 -right-0.5 w-1.5 h-1.5 sm:w-2 sm:h-2 bg-green-500 rounded-full ring-1 ring-gray-900"></div>
                </div>
                <div className="min-w-0">
                  <p className="text-[10px] sm:text-xs font-medium text-white leading-none truncate max-w-[60px] sm:max-w-[80px]">
                    {ownerName}
                  </p>
          ...
          {/* Hover Overlay CTA */}
          <div className="absolute inset-x-0 bottom-0 transform translate-y-full group-hover:translate-y-0 transition-transform duration-300">
            <div className="bg-gradient-to-r from-purple-600 to-indigo-600 p-1.5 sm:p-2 text-center">
