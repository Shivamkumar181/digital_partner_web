...
const Certificates = () => {
  ...
  return (
    <div className="space-y-4 sm:space-y-6 min-h-screen bg-black p-2 sm:p-4 lg:p-6 rounded-2xl">
      {/* Header */}
      <div className="bg-gradient-to-r from-indigo-600 to-purple-600 rounded-xl p-4 sm:p-6 text-white">
        <h1 className="text-2xl sm:text-3xl font-bold mb-1 sm:mb-2">My Certificates</h1>
      ...
      {certificates.length === 0 ? (
        <div className="bg-gray-950 border border-gray-800 rounded-xl shadow-lg p-8 sm:p-12 text-center">
          <div className="text-5xl sm:text-6xl mb-4">🎖️</div>
          <h3 className="text-lg sm:text-xl text-white font-semibold mb-2">No Certificates Yet</h3>
          <p className="text-sm sm:text-base text-gray-400 mb-6">
            Start contributing to projects to earn verified certificates
          </p>
          <a href="/projects" className="inline-block bg-gradient-to-r from-indigo-500 to-purple-600 text-white px-6 py-2.5 rounded-lg font-medium hover:shadow-lg transition-all duration-200">
            Browse Projects
          </a>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          {certificates.map((cert, index) => (
            ...
            >
              {/* Certificate Header */}
              <div className="bg-gradient-to-r from-indigo-500 to-purple-500 p-3 sm:p-4">
                <div className="flex justify-between items-start">
                  <div className="text-white">
                    <p className="text-xs sm:text-sm opacity-90">Certificate of Achievement</p>
                    <p className="text-sm sm:text-lg font-bold truncate max-w-[120px] sm:max-w-[180px]">{cert.certificateId}</p>
                  </div>
                  <QrCode className="text-white flex-shrink-0" size={28} />
                </div>
              </div>
              
              {/* Certificate Body */}
              <div className="p-4 sm:p-6">
                <h3 className="text-lg sm:text-xl text-white font-bold mb-2 line-clamp-1">{cert.projectId?.title}</h3>
                <p className="text-sm sm:text-base text-gray-400 mb-3 line-clamp-2">
                  Contribution: {cert.contributionId?.description?.substring(0, 100)}...
                </p>
                ...
                <div className="flex flex-col sm:flex-row gap-2">
                  <button
                    ...
                    className="flex-1 flex items-center justify-center gap-2 bg-gray-800 text-gray-100 border border-gray-700 rounded-lg px-3 py-2 text-xs sm:text-sm hover:bg-gray-700 transition"
                  >
                    View
                  </button>
                  <button
                    onClick={() => window.open(cert.pdfUrl || `/verify-certificate/${cert.certificateId}`, '_blank')}
                    className="flex-1 flex items-center justify-center gap-2 bg-gradient-to-r from-indigo-500 to-purple-600 text-white rounded-lg px-3 py-2 text-xs sm:text-sm hover:shadow-lg transition-all duration-200"
                  >
                    <Download size={16} />
                    Download
                  </button>
                ...
                {/* Verification Link */}
                <div className="mt-4 pt-4 border-t border-gray-800">
                  <p className="text-[10px] sm:text-xs text-gray-500 text-center truncate">
                    Verify: {window.location.origin}/verify/{cert.certificateId}
                  </p>
                </div>
              </div>
            </motion.div>
